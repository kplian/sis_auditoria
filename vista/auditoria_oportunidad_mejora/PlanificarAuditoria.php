...
<script>
    ...
    Phx.vista.PlanificarAuditoria = {
        ...
        onCrearFormulario: function () {
            ...
            this.storePuntoNorma = new Ext.data.GroupingStore({
                ...
                columns: [
                    new Ext.grid.RowNumberer(),
                    {
                        ...
                        renderer: function (value, p, record) {
                            return String.format('<a style="cursor:pointer;">{0}</a>', record.data['proceso']);
                        },
                    },
                    {
                        header: 'Responsable',
            ...
            const responsable = new Ext.grid.GridPanel({
                ...
                columns: [
                    new Ext.grid.RowNumberer(),
                    {
                        ...
                        sortable: false,
                        renderer: function (value, p, record) {
                            return String.format('<a style="cursor:pointer;">{0}</a>', record.data['desc_funcionario1'])
                        },
                    },
                    {
                        header: 'Tipo Auditor',
            ...
            const pregunta = new Ext.grid.GridPanel({
                ...
                columns: [
                    new Ext.grid.RowNumberer(),
                    {
                        ...
                        sortable: false,
                        renderer: function (value, p, record) {
                            return String.format('<a>{0}</a>', record.data['descrip_pregunta'])
                        },
                    },
                    {
                        header: 'Estado Reg.',
                    ...
                    },
                    {
                        ...
                        items: [{
                            ...
                            handler: function (grid, rowIndex, colIndex) {
                                const rec = sef.storeCronograma.getAt(rowIndex);
                                console.log(rec)
                                Ext.Ajax.request({
                                    url: '../../sis_auditoria/control/Cronograma/eliminarCronograma',
                                    params: {
                                        id_cronograma: rec.get('id_cronograma')
                                    },
                                    isUpload: false,
                                    success: function (a, b, c) {
                                        sef.storeCronograma.load();
                                    },
        ...
        formularioEquipo: function () {
            ...
            const tienda = new Ext.data.JsonStore({
                ...
                // listeners: { 'load': function(store) { store.filter( 'desc_funcionario1', null ); } }
            });
            const isForm = new Ext.form.FormPanel({
                labelAlign: 'top',
                frame: true,
                        ...
                        items: [{
                            ...
                            items: [{
                                ...
                                store: new Ext.data.JsonStore({
                                    ...
                                    remoteSort: true,
                                    baseParams: {
                                        par_filtro: 'ptonor.nombre_pn#ptonor.codigo_pn',
                                        dir: 'ASC',
                                        sort: 'id_pn',
                                        limit: '100',
                                        start: '0'
                ...
                columns: [
                    new Ext.grid.RowNumberer(),
                    {
                        ...
                        items: [{
                            ...
                            tooltip: 'Sell stock',
                            handler: function (grid, rowIndex, colIndex) {
                                const rec = tienda.getAt(rowIndex);
                                Ext.Ajax.request({
                                    url: '../../sis_auditoria/control/Cronograma/eliminarCronograma',
                                    params: {
                                        id_cronograma: rec.get('id_cronograma')
        ...
        asignarPregunta: function (formulario) {
            ...
            isForm.getForm().items.items[0].on('select', function (combo, record, index) {
                isForm.getForm().items.items[1].store.baseParams = {
                    dir: 'ASC',
                    sort: 'id_pn',
                    limit: '100',
                    start: '0',
