    ...
    ***********************************/
    ...
    if (p_transaccion = 'SSOM_AOM_INS') then
        ...
        begin
            ...
            if (v_parametros.id_tipo_auditoria != 2) then
                select tip.codigo_tpo_aom
                into v_codigo_aud
                from ssom.ttipo_auditoria tip
                where tip.id_tipo_auditoria = v_parametros.id_tipo_auditoria;
            else
                select pa.codigo_parametro
                into v_codigo_aud
                from ssom.tparametro pa
                where pa.id_parametro = v_parametros.id_tipo_om;
            end if;
            ...
            if (pxp.f_existe_parametro(p_tabla,'id_funcionario')) then
                v_funcionario = v_parametros.id_funcionario;
            end if;
            ...
            if v_funcionario is not null then
                --Sentencia de la insercion
            ...
            v_resp = pxp.f_agrega_clave(v_resp, 'mensaje',
                                        'Auditoria - Oportunidad Mejora almacenado(a) con exito (id_aom' || v_id_aom ||
                                        ')');
        ...
        ***********************************/
        ...
        begin
            ...
            update ssom.tauditoria_oportunidad_mejora
        ...
        ***********************************/
        ...
        begin
            ...
            update ssom.tauditoria_oportunidad_mejora
            set recomendacion = v_parametros.recomendacion
            where id_aom = v_parametros.id_aom;
        ...
        ***********************************/
        ...
        begin
            ...
            select ao.id_tipo_auditoria, ao.id_gestion
            into v_auditria
            from ssom.tauditoria_oportunidad_mejora ao
            where ao.id_aom = v_parametros.id_aom;

            select tip.codigo_tpo_aom
            into v_codigo_aud
            from ssom.ttipo_auditoria tip
            where tip.id_tipo_auditoria = v_auditria.id_tipo_auditoria;

            select co.id_corre,
                   co.nro_actual,
                   co.nro_siguiente
            into v_correlativo_del
            from ssom.tcorrelativo co
            where co.codigo_correlativo = v_codigo_aud
              and co.id_gestion = v_auditria.id_gestion;


            update ssom.tcorrelativo
            set nro_actual    = v_correlativo_del.nro_actual - 1,
                nro_siguiente = v_correlativo_del.nro_siguiente - 1
            where id_corre = v_correlativo_del.id_corre;
            ...
            delete
            from ssom.tauditoria_npn nk
            where nk.id_aom = v_parametros.id_aom;

            delete
            from ssom.taom_riesgo_oportunidad oa
        ...
        begin
            ...
            if va_codigo_estado[1] = 'ejecutada' then
                ...
                if (v_registros_proc.resumen is null or v_registros_proc.resumen = '') then
                    ...
                    for v_equipo_record in (select fun.desc_funcionario1
                                            ...
                                            where re.id_aom = v_registros_proc.id_aom)
                        loop
            ...
            if va_codigo_estado[1] = 'curso' then
                ...
                for v_no_conformindad in (select nc.id_nc,
                                          ...
                                          where nc.id_aom = v_registros_proc.id_aom)
                    loop
    ...
    elseif (p_transaccion = 'SSOM_WFBACK_IME') then
        ...
        begin
            ...
            if v_codigo_estado_siguiente not in ('borrador', 'finalizado', 'anulado') then
                v_acceso_directo =
                        '../../../sis_auditoria/vista/auditoria_oportunidad_mejora/AuditoriaOportunidadMejora.php';
    ...
    elseif (p_transaccion = 'SSOM_WFNEXT_INS') then
        ...
        begin
            ...
            v_resp = pxp.f_agrega_clave(v_resp, 'mensaje', 'Se realizo el cambio de estado del pago simple id=' ||
                                                           v_id_proceso_wf::varchar);
            v_resp = pxp.f_agrega_clave(v_resp, 'operacion', 'cambio_exitoso');
            -- Devuelve la respuesta
            return v_resp;
        end;
                ...
                for v_json_formulario_record in (select json_array_elements(v_json_formularion))
                    loop
                        ...
                        if v_parametros.informe = 'si' then
                            ...
                            update ssom.tauditoria_oportunidad_mejora
                            set id_destinatario = v_id_destinatario,
                            ...
                            delete
                            from ssom.tdestinatario dd
                            where dd.id_aom = v_parametros.id_aom;

                            foreach v_id_funcionario IN array (string_to_array(v_id_destinatarios, ','))
                                loop

                                    --Sentencia de la insercion
                                    insert into ssom.tdestinatario(id_aom,
                                                                   id_funcionario,
                                                                   estado_reg,
                                                                   id_usuario_ai,
                                                                   fecha_reg,
                            ...
                            update ssom.tauditoria_oportunidad_mejora
                            set fecha_prog_inicio = v_fecha_prog_inicio,
      ...
      ***********************************/
        ...
        begin
            ...
            if (v_parametros.id_tipo_auditoria != 0) then
                select tip.codigo_tpo_aom
                into v_codigo_aud
                from ssom.ttipo_auditoria tip
                where tip.id_tipo_auditoria = v_parametros.id_tipo_auditoria;
            else
                v_codigo_aud = v_parametros.codigo;
            end if;


            v_correlativo = ssom.f_generar_correlativo(v_codigo_aud, EXTRACT(YEAR FROM current_date)::integer, false);
            --raise exception '%',v_correlativo;
            --Definicion de la respuesta
            v_resp = pxp.f_agrega_clave(v_resp, 'mensaje', 'Datos de decuentos recuperados con exito');
