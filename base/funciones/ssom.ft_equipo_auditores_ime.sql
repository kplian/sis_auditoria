CREATE OR REPLACE FUNCTION ssom.ft_equipo_auditores_ime (
...
BEGIN
	...
	***********************************/
	...
	if(p_transaccion='SSOM_EUS_INS')then
        ...
        begin
        	...
        	insert into ssom.tequipo_auditores(
			estado_reg,
			-- obs_dba,
			id_funcionario,
            ...
            for v_record in (select n.id_norma
                            from ssom.tnorma n
                            where n.estado_reg = 'activo')loop
			...
			v_resp = pxp.f_agrega_clave(v_resp,'mensaje','Equipo Aduitores almacenado(a) con exito (id_equipo_auditores'||v_id_equipo_auditores||')');
	...
	***********************************/
		...
		begin
			...
			delete from ssom.tequipo_auditores
            where id_equipo_auditores=v_parametros.id_equipo_auditores;
...
ALTER FUNCTION ssom.ft_equipo_auditores_ime (p_administrador integer, p_id_usuario integer, p_tabla varchar, p_transaccion varchar)
  OWNER TO postgres;